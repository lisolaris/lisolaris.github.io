...
开环控制系统指的是系统的控制作用不受输出影响的控制系统。一个典型的例子是开车时控制车速，在不考虑刹车的情况下驾驶员踩油门越深，车速就越快。虽然理想状况下车速的快慢和驾驶员踩下油门的深浅是完全对应的，油门踩到某个程度车速就会固定在一个值；但实际上我们知道路况、天气、车辆载重等因素都会对车速产生影响。这就是扰动对开环控制作用的干扰。

闭环控制系统则将系统输出量的信息反馈到输入端，得到输入和输出的差值（称作偏差）；将偏差输入到控制系统，
