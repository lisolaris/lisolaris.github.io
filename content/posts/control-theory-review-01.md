...
categories: ["自动控制原理"]
description: "自动控制原理的学习笔记。本篇主要介绍了自动控制的一般概念，包括开环控制与闭环、控制系统的分类方式、控制系统的性能目标；控制系统的数学模型，包括电路系统中的网孔法、节点电压法，机械系统所遵循的物理规律；傅里叶级数和傅里叶变换，从傅里叶变换到拉普拉斯变换，以及使用拉普拉斯变换求解常微分方程。最后给出了一些自动控制理论方面的参考书目。"
toc: true
draft: false
---
...
这一部分内容主要来源于卢老师《自动控制原理》的`附录A 拉普拉斯变换及反变换`。如果仍有不理解的地方，可以参考胡浩基老师的信号与系统课程中关于部分分式法的片段：[用傅里叶变换计算信号卷积_2022浙江大学信号与系统（含配套课件和代码） - 胡浩基老师](https://www.bilibili.com/video/BV1g94y1Q76G?t=829.6&p=23)；胡老师在他的课程里把这种方法称为『盖起来算其他』，很简明易懂的称呼。
...
[下一篇文章]({{<siteurl>}}articles/2025/03/control-theory-review-02/)预计介绍一下传递函数、梅逊增益公式等等把第一二章完结，之后就该进入时域分析、频域分析和根轨迹法的正题了。
