...
满足必要条件的一、二阶系统一定稳定。但对于高阶系统，单纯满足必要条件不一定稳定，还需要使用劳斯判据。

#### 劳斯判据

劳斯判据需要列出如下格式的劳斯表。表中的前两行分别由特征方程奇/偶项系数构成，其他各行根据前两行的数值计算。
...
劳斯表计算过程中有时会出现某行的第一列元素为 $0$ 的情况，若按原规则计算下一行会出现除零的情况。此时可以用一个符号 $\epsilon$ 代替 $0$ 参与计算并列出代数式，最后计算各个算式取 $\epsilon \to 0$ 的极限过程时的结果得到劳斯表。
