...
这部分内容可以参考*胡寿松 《自动控制原理（第七版）》 102页 高阶系统闭环主导极点及其动态性能分析*。
...
为了使高阶系统的性能尽量优秀，一般会通过调整增益使得系统具有一对共轭的闭环主导极点[^4]，从而可以使用二阶系统的动态性能指标估算高阶系统的动态性能。一般规定，**若某极点的实部大于主导极点实部的5～6倍以上时，则可以忽略相应分量的影响；若两相邻零、极点间的距离比它们本身的模值小一个数量级（10倍以上）时，则称该零、极点对为“偶极子”，其作用近似抵消，可以忽略相应分量的影响。**

但二阶近似并不能完全反映高阶系统的动态性能，额外的零/极点将会带来如下影响：

1. **闭环零点影响**：闭环零点的存在会减小峰值时间，使系统响应速度加快，超调量 $\sigma \char37$ 增大。这表明闭环零点会减小系统阻尼，并且这种作用将随闭环零点接近虚轴而加剧。因此，配置闭环零点时，要折中考虑闭环零点对系统响应速度和阻尼程度的影响。

2. **闭环非主导极点影响**：非主导极点会增大峰值时间，使系统响应速度变缓，但可以使超调量 $\sigma \char37$ 减小。这表明闭环非主导极点可以增大系统阻尼，且这种作用将随极点接近虚轴而加剧。

有关高阶系统中零极点位置对系统性能的影响将会在 [下一篇文章]({{<siteurl>}}/articles/2025/11/control-theory-review-04/) 中展开具体讨论；此外还可以参考下面的 [估算高阶系统动态性能指标的零点极点法](#估算高阶系统动态性能指标的零点极点法)。
...
式中 $A_0 = \displaystyle \lim_{s \to 0} s C(s) = \frac{M(0)}{D(0)}, A_j = \lim_{s \to j} (s - \lambda_j) C(s)$ 是 $C(s)$ 在闭环实极点 $\lambda_j$ 处的留数[^5]； $B_k$ 和 $C_k$ 是 $C(s)$ 在闭环复极点 $- \zeta_k \omega_k \pm \mathrm{j} \omega_k \sqrt{1 - \zeta_k^2}$ 处的留数有关的常系数。对上式做拉普拉斯反变换可得
...
在绝大多数实际系统的闭环零、极点中，可以选留最靠近虚轴的一个或几个极点作为主导极点，略去比主导极点距虚轴远5倍以上的闭环零、极点，以及不十分接近虚轴的相互靠得很近的偶极子，忽略其对系统动态性能的影响，然后按下表中相应的公式估算高阶系统的动态性能指标。
...
式中 $A_i = \displaystyle \lim_{s \to \lambda_i} (s - \lambda_i) C(s)$ 是 $C(s)$ 在闭环极点 $\lambda_i$ 处的留数。对上式进行拉普拉斯反变换，得系统的单位脉冲响应函数[^6]
...
线性定常系统如果稳定，则它一定是大范围稳定[^7]的，且原点是其唯一的平衡点。
...
[^4]: 可以证明，对于一般的实系数线性控制系统（所有物理量、元件参数为实数），其复平面上的极点 / 零点总是成对出现。

[^5]: 有关留数法解部分分式，可以回顾 [部分分式法展开]({{<siteurl>}}/articles/2025/03/control-theory-review-01/#部分分式法展开) 。

[^6]: [传递函数的拉普拉斯反变换即为系统的脉冲响应]( {{<siteurl>}}/articles/2025/03/control-theory-review-02/#传递函数的性质 )（即系统在单位脉冲信号 $\delta(t)$ 激励下产生的输出）。

[^7]: 相对于对非线性系统做 **小偏差线性化** 后求解出的系统参数只在所研究的工作点附近准确，线性定常系统在参数变化的一个较大范围内都具有稳定性。
